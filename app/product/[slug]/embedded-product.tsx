...
export function EmbeddedProduct({ productRef, title, official }: { productRef: string; title: string; official: boolean }) {
  ...
  useEffect(() => {
    setState("loading");
    const timeout = window.setTimeout(() => setState((current) => current === "loading" ? "failed" : current), 12000);
      ...
      {state !== "ready" && (
        <div className={`embedded-product-status ${state}`} role="status">
          {state === "loading" ? (
            ...
            <><strong>作品暂时没有响应</strong><span>可以重新载入，或在独立页面中打开。</span><button type="button" onClick={() => { setReloadKey((key) => key + 1); setState("loading"); }}><RotateCcw size={15} />重新载入</button></>
          )}
        </div>
      )}
      <iframe
