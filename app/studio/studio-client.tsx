...
type StudioProduct = { id: number; title: string; category: string; imageUrl?: string | null; coverTheme: string; plays: number; likes: number; status: string; createdAt: string };
...
export function StudioClient() {
  ...
  return <div className="studio-page">
    ...
    <section className="studio-metrics"><div><span><Eye size={16} /> 总体验</span><strong><AnimatedNumber value={totals.plays} /></strong><small>{products.length ? `${products.length} 件已发布作品` : "发布后开始记录"}</small><i className="metric-sparkline one" /></div><div><span><Heart size={16} /> 收到喜欢</span><strong><AnimatedNumber value={totals.likes} /></strong><small>{totals.plays ? `${Math.round(totals.likes / totals.plays * 100)}% 体验者留下喜欢` : "等待第一位体验者"}</small><i className="metric-sparkline two" /></div><div><span><Users size={16} /> 作品数量</span><strong><AnimatedNumber value={products.length} /></strong><small>{draft ? "另有 1 份本机草稿" : "没有未发布草稿"}</small><i className="metric-sparkline three" /></div><div><span><Zap size={16} /> 累计获得</span><strong><AnimatedNumber value={wallet.lifetimeEarned} suffix=" 果" /></strong><small>来自发布、签到与支持</small><i className="metric-sparkline four" /></div></section>
    <div className="studio-grid"><section className="studio-panel analytics-panel"><div className="panel-heading"><span><BarChart3 size={17} /> 作品{metric}对比</span><div>{(["体验", "喜欢"] as const).map((item) => <button key={item} className={metric === item ? "active" : ""} onClick={() => setMetric(item)}>{item}</button>)}</div></div><div className="analytics-chart"><div className="chart-grid">{Array.from({ length: 5 }).map((_, index) => <i key={index} />)}</div><div className="chart-bars">{bars.map((height, index) => <span key={index} style={{ height }} title={orderedProducts[index] ? `${orderedProducts[index].title}: ${values[index]}` : "暂无数据"}><b /></span>)}</div><div className="chart-labels"><span>{orderedProducts[0]?.title ?? "暂无作品"}</span><span>{orderedProducts.at(-1)?.title ?? "发布后显示"}</span></div></div></section><aside className="studio-panel release-panel"><div className="panel-heading"><span><Radio size={17} /> 最近作品</span><Link href="/feed">发布动态</Link></div>{orderedProducts.slice(0, 3).map((item) => <Link className="release-row" href={`/product/${item.id}`} key={item.id}><i className={item.coverTheme} /><span><strong>{item.title}</strong><small>{item.createdAt}</small></span></Link>)}{orderedProducts.length === 0 && <div className="studio-panel-empty">发布第一件作品后，版本会在这里汇总。</div>}</aside></div>
    <Reveal className="studio-projects"><div className="deep-section-heading"><div><span className="deep-eyebrow">YOUR WORKS / {String(products.length + (draft ? 1 : 0)).padStart(2, "0")}</span><h2>作品与草稿</h2></div>{products.length > 1 && <button className={manage ? "active" : ""} onClick={() => setManage((value) => !value)}>{manage ? <><Check size={14} /> 保存顺序</> : "管理顺序"}</button>}</div><div className="studio-project-table">{orderedProducts.map((product, index) => <div className="studio-project-row" key={product.id}><Link href={`/product/${product.id}`}><span className="project-index">{String(index + 1).padStart(2, "0")}</span>{product.imageUrl ? <img src={product.imageUrl} alt="" /> : <span className={`studio-project-placeholder ${product.coverTheme}`} /> }<span className="project-main"><strong>{product.title}</strong><small>{product.createdAt} · {product.category}</small></span><span><Eye size={14} /> {product.plays.toLocaleString()}</span><span><Heart size={14} /> {product.likes}</span><em className="live">已发布</em><ArrowRight size={16} /></Link>{manage && <span className="project-order-controls"><button onClick={() => move(index, -1)} disabled={index === 0} aria-label="上移"><ArrowUp size={14} /></button><button onClick={() => move(index, 1)} disabled={index === orderedProducts.length - 1} aria-label="下移"><ArrowDown size={14} /></button></span>}</div>)}{draft && <div className="studio-project-row studio-draft-row"><Link href="/studio/new"><span className="project-index">DR</span>{draft.imageUrl ? <img src={draft.imageUrl} alt="" /> : <span className={`studio-project-placeholder ${draft.theme ?? "coral"}`} />}<span className="project-main"><strong>{draft.title || "未命名草稿"}</strong><small>本机自动保存 · {draft.category || "未分类"}</small></span><span /><span /><em className="draft">继续编辑</em><ArrowRight size={16} /></Link></div>}</div>{signedIn === false && <div className="studio-empty"><LogIn size={25} /><strong>登录后管理你的作品</strong><p>发布、数据、草稿和果子记录会集中到当前账号。</p><Link className="primary-action" href="/signin?return_to=%2Fstudio">登录造场</Link></div>}{signedIn === true && products.length === 0 && !draft && <div className="studio-empty"><Sparkles size={25} /><strong>这里还没有作品</strong><p>从一个能被体验的最小版本开始。</p><Link className="primary-action" href="/studio/new">创建第一件作品</Link></div>}</Reveal>
  </div>;
}
