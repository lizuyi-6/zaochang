...
import { normalizeProductTheme, normalizePricingModel, PRICING_MODELS, PRODUCT_THEMES, type PricingModel, type ProductTheme } from "../../lib/product-policy";

const steps = ["基本信息", "体验设置", "提交预审"];

const pricingChoices = [
  { id: "free", title: "免费开放", text: "所有人直接进入" },
  { id: "one_time", title: "一次解锁", text: "支付一次，持续访问" },
  { id: "per_use", title: "按次体验", text: "每次进入单独结算" },
] as const satisfies readonly { id: (typeof PRICING_MODELS)[number]; title: string; text: string }[];

// 定价与主题选项由共享白名单驱动,创建向导不再复制数据库策略。
const pricingModelChoices = PRICING_MODELS.map((id) => pricingChoices.find((choice) => choice.id === id)!);
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      try {
        ...
        setTitle(String(draft.title ?? "")); setDescription(String(draft.description ?? "")); setCategory(String(draft.category ?? "互动体验")); setPricingModel(normalizePricingModel(draft.pricingModel)); setPrice(Number(draft.price ?? 0)); setDemoUrl(String(draft.demoUrl ?? "")); setImageUrl(String(draft.imageUrl ?? "")); setTheme(normalizeProductTheme(draft.theme));
