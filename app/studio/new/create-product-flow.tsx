...
export function CreateProductFlow() {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      try {
        ...
        setTitle(String(draft.title ?? "")); setDescription(String(draft.description ?? "")); setCategory(String(draft.category ?? "互动体验")); setPrice(Number(draft.price ?? 0)); setDemoUrl(String(draft.demoUrl ?? "")); setImageUrl(String(draft.imageUrl ?? "")); setTheme(String(draft.theme ?? "coral"));
  ...
  useEffect(() => {
    localStorage.setItem("zaochang-product-draft", JSON.stringify({ title, description, category, price, demoUrl, imageUrl, theme }));
  }, [category, demoUrl, description, imageUrl, price, theme, title]);
  ...
  const submit = async (event: FormEvent) => {
    ...
    setSubmitting(true);
    const response = await fetch("/api/products", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ title, description, category, price, demoUrl, imageUrl, coverTheme: theme }) });
