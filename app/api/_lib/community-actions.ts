// 社区动作域模块:POST /api/actions 的各用例。
// experience 播放计数是匿名分支(仅 IP 限流),单独出口;其余九个分支需要登录,
...
export async function handleExperienceAction(request: Request, input: Record<string, unknown>): Promise<Response> {
  ...
  // 该分支刻意放在登录守卫之前，仅受 IP 限流（120/h）约束。
...
export async function handleMemberAction(member: MemberIdentity, input: Record<string, unknown>): Promise<Response> {
