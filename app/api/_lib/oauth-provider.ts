...
export async function createDeveloperClient(ownerEmail: string, input: Record<string, unknown>) {
  ...
  return { clientId, clientSecret, name, clientType, allowedScopes: canonicalScopes(requestedScopes), redirectUris };
}
...
export async function createAuthorizationRequest(url: URL, userEmail: string) {
  ...
  if (!hasScopes(client.allowedScopes, scopes)) throw new OAuthProviderError("invalid_scope", 400);
  ...
  return { requestToken, client, scopes };
}
...
async function authenticateTokenClient(request: Request, params: URLSearchParams) {
  ...
  const client = await clientById(clientId);
  if (!client || client.status !== "active") throw new OAuthProviderError("invalid_client", 401);
...
async function issueTokens(origin: string, client: ClientRow, userEmail: string, scope: string, nonce: string | null, source: { codeHash?: string; refreshHash?: string }) {
  ...
  const statements = [
    ...
    db.prepare(
      `INSERT INTO oauth_provider_refresh_tokens
       ...
       VALUES (?, ?, ?, ?, ?)`,
    ).bind(refreshHash, client.clientId, userEmail, scope, expiresIn(REFRESH_TOKEN_SECONDS)),
  ];
  if (source.codeHash) {
...
export async function exchangeToken(request: Request, params: URLSearchParams) {
  ...
  if (grantType === "refresh_token") {
    ...
    const row = await database().prepare(
      `SELECT client_id AS clientId, user_email AS userEmail, scope, expires_at AS expiresAt, revoked_at AS revokedAt
       FROM oauth_provider_refresh_tokens WHERE token_hash = ?`,
    ).bind(refreshHash).first<{ clientId: string; userEmail: string; scope: string; expiresAt: string; revokedAt: string | null }>();
    ...
    try {
      return await issueTokens(origin, client, row.userEmail, scope, null, { refreshHash });
    } catch {
      throw new OAuthProviderError("invalid_grant", 400);
    }
  }
  ...
  const row = await database().prepare(
    `SELECT t.token_hash AS tokenHash, t.client_id AS clientId, c.name AS clientName,
     ...
     WHERE t.token_hash = ? AND t.revoked_at IS NULL AND t.expires_at > CURRENT_TIMESTAMP
       AND c.status = 'active'`,
  ).bind(tokenHash).first<TokenIdentity>();
