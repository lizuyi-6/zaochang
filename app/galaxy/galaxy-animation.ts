...
import type { GalaxyInteraction } from "./galaxy-interaction";
import type { GalaxySceneRuntime } from "./galaxy-scene";

export type GalaxyAnimationOptions = {
  runtime: GalaxySceneRuntime;
  cameraController: CameraController;
  interaction: GalaxyInteraction;
  prefersReducedMotion: boolean;
  targetRef: { current: TargetId };
  resetRef: { current: number };
  warpRef: { current: number };
  pausedRef: { current: boolean };
  quietRef: { current: boolean };
  cruiseRef: { current: boolean };
  passageRef: { current: number };
  onReady: () => void;
};
...
export function createGalaxyAnimation({
  ...
  passageRef,
  onReady,
}: GalaxyAnimationOptions): GalaxyAnimation {
  const {
