...
export default async function ProfilePage() {
  ...
  return <div className="profile-page">
    ...
    <section className={`profile-identity${founder ? " founder-profile" : ""}`}><span className="profile-large-avatar">{name[0]}</span><div><span className="deep-eyebrow">{founder ? <><BadgeCheck size={14} /> FOUNDER / ZAOCHANG</> : <><Sparkles size={14} /> MAKER PROFILE</>}</span><h1>{name}{founder && <small className="profile-founder-badge"><BadgeCheck size={13} /> 造场创始人</small>}</h1><p>{profile?.bio}</p><div><span><MapPin size={14} /> {profile?.location}</span>{website && <a href={websiteHref} target="_blank" rel="noreferrer"><Link2 size={14} /> {website}</a>}</div></div><div className="profile-actions">{founder && <Link href="/founder"><BadgeCheck size={16} /> 创始人中心</Link>}{admin && <Link href="/admin"><ShieldCheck size={16} /> 平台管理</Link>}<Link href="/profile/edit"><Settings size={16} /> 编辑主页</Link></div></section>
