...
- `HYPERKNOW_*` 三件可选(Hyperknow 学习 Agent,见 `HYPERKNOW.md`):`HYPERKNOW_AI_MODEL` 缺省回落 `AI_CHAT_MODEL` 再回落 `step-explore`;`HYPERKNOW_AI_BASE_URL`/`HYPERKNOW_AI_API_KEY` 覆盖位缺省回落 `AI_CHAT_BASE_URL`/`AI_CHAT_API_KEY`(该上游必须支持 Anthropic Messages 协议 `/messages`);`HYPERKNOW_TTS_BASE_URL`/`HYPERKNOW_TTS_MODEL` 缺省 `https://api.stepfun.com/v1` + `stepaudio-3-tts`(StepAudio 3 代 TTS,兼容既有克隆 voice-tone 与 speed)。共享 AI_CHAT_* 密钥面,不新增必需 secret。
